...
def process_and_redact(image_path, output_path, parent_window):
    ...
    for index, line in enumerate(line_items):
        ...
        if user_choices['id_num'] and any(k in norm for k in ['ma so bhyt', 'so the bhyt', 'ma the bhyt']):
            for item in line['items']:
                digits_only = re.sub(r'\D', '', item['text'])
                is_date_like = bool(re.search(date_pattern, item['text']))
                if not is_date_like and len(digits_only) >= 8:
                    redact_boxes.append((item['x1'], item['y1'], item['x2'], item['y2']))
            continue
        ...
        if user_choices['id_num']:
            digits_only = re.sub(r'\D', '', text)
            is_date_line = bool(re.search(date_pattern, text))

            if document_type == "Thẻ Bảo hiểm y tế":
                if not is_date_line and len(digits_only) in [10, 12, 15, 16] and len(text) <= 25:
                    is_sensitive = True
            else:
